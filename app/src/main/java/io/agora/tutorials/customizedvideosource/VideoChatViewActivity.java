...
public class VideoChatViewActivity extends AppCompatActivity {
    ...
    private static final int PERMISSION_REQ_ID_EXTERNAL_STORE = PERMISSION_REQ_ID_CAMERA + 1;
    ...
    public void onRequestPermissionsResult(int requestCode,
        ...
        switch (requestCode) {
            ...
            case PERMISSION_REQ_ID_CAMERA: {
                if (grantResults.length > 0
                        && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE, PERMISSION_REQ_ID_EXTERNAL_STORE);
                    initAgoraEngineAndJoinChannel();
                } else {
            ...
            case PERMISSION_REQ_ID_EXTERNAL_STORE:{
                if (grantResults.length > 0
                        && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                } else {
...
//        mRtcEngine.setChannelProfile(Constants.CHANNEL_PROFILE_COMMUNICATION);
    ...
    private void setupVideoProfile() {
        ...
        //    mRtcEngine.setExternalVideoSource(true, true, true);
            android.util.Log.v("longxin","TextureEncode Supported");
        } else {
        //    throw new RuntimeException("Can not work on device do not supporting texture" + mRtcEngine.isTextureEncodeSupported());
            android.util.Log.v("longxin","TextureEncode not Supported");
        }
        mRtcEngine.setExternalVideoSource(true, false, true);//add by longxin
//        mRtcEngine.setVideoProfile(Constants.VIDEO_PROFILE_480P, true);
    ...
    private CustomizedCameraRenderer setupLocalVideo(Context ctx) {
        ...
        mCustomizedCameraRenderer.setOnFrameAvailableHandler(new CustomizedCameraRenderer.OnFrameAvailableListener() {
            @Override
            public void onFrameAvailable(/*int texture, EGLContext eglContext, int rotation,*/ byte[] bytes,int width,int height) {

                AgoraVideoFrame vf = new AgoraVideoFrame();
                //vf.format = AgoraVideoFrame.FORMAT_NV21;
                vf.format = AgoraVideoFrame.FORMAT_RGBA;
                vf.timeStamp = System.currentTimeMillis();
                vf.stride = width;//1080;
                vf.height = height;//1920;
                ...
                //add by longxin
                if(testbool){
                    CustomizedCameraRenderer.savePNGBitmap(CustomizedCameraRenderer.loadARGBBitmapFromRGBAByteArray(bytes,width,height,false),"/sdcard/screen2.png");
                    testbool = false;
                }
                //add by longxin
                android.util.Log.v("longxin","debug---5---");
                boolean result = mRtcEngine.pushExternalVideoFrame(vf);
                android.util.Log.v("longxin","debug---6---");
                if (DBG) {
                    Log.d(LOG_TAG, "onFrameAvailable " + " result" + result );
                }
    ...
    private void joinChannel() {
        ...
        mRtcEngine.joinChannel(null, "qaz001", "Extra Optional Data", 0); // if you do not specify the uid, we will generate the uid for you
    }
